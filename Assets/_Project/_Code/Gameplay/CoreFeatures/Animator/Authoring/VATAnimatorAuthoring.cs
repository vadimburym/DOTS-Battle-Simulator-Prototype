...
namespace VATDots
{
    ...
    public sealed class VATAnimatorAuthoring : MonoBehaviour
    {
        ...
        public void ApplyLibraryToSharedMaterial()
        {
            ...
            if (library.positionTexture != null)
            {
                float invW = 1f / Mathf.Max(1, library.positionTexture.width);
                float invH = 1f / Mathf.Max(1, library.positionTexture.height);
                sharedMaterial.SetVector("_VATPosTexInfo", new Vector4(invW, invH, library.positionTexture.width, library.positionTexture.height));
            }

            if (library.metadataTexture != null)
            {
                float invW = 1f / Mathf.Max(1, library.metadataTexture.width);
                float invH = 1f / Mathf.Max(1, library.metadataTexture.height);
                sharedMaterial.SetVector("_VATMetaTexInfo", new Vector4(invW, invH, library.metadataTexture.width, library.metadataTexture.height));
            }
        }
        ...
        private sealed class Baker : Baker<VATAnimatorAuthoring>
        {
            public override void Bake(VATAnimatorAuthoring authoring)
            {
                ...
                if (authoring.overrideMesh != null)
                    DependsOn(authoring.overrideMesh);

                if (authoring.targetRenderer != null)
                    DependsOn(authoring.targetRenderer);

                DependsOnComponentsInChildren<MeshFilter>();
                DependsOnComponentsInChildren<SkinnedMeshRenderer>();
                DependsOnComponentsInChildren<Renderer>();
                ...
                if (authoring.overrideMesh != null)
                {
                    mesh = authoring.overrideMesh;
                }
                else if (authoring.targetRenderer != null)
                {
                    if (authoring.targetRenderer is SkinnedMeshRenderer targetSmr)
                    {
                        mesh = targetSmr.sharedMesh;
                    }
                    else
                    {
                        var targetMf = GetComponent<MeshFilter>(authoring.targetRenderer);
                        if (targetMf != null)
                ...
                else
                {
                    ...
                    if (mesh == null)
                    {
                        var mfChild = GetComponentInChildren<MeshFilter>();
                        if (mfChild != null)
                            mesh = mfChild.sharedMesh;
                    }

                    if (mesh == null)
                    {
                        var smrChild = GetComponentInChildren<SkinnedMeshRenderer>();
                        if (smrChild != null)
                            mesh = smrChild.sharedMesh;
                    }
                }
